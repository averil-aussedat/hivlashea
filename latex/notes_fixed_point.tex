...
\myproof{
	...
	\begin{align*}
		n_i(x) = 2 \nu \int_{v=-\ve}^{0} \int_{z=x_b(x,v)}^{1} \frac{f_e\left(z, -\left(v^2 + 2\left(\varphi(x) - \varphi(z)\right)\right)^{1/2}\right)}{\left(v^2 + 2\left(\varphi(x) - \varphi(z)\right)\right)^{1/2}} \, dz dv.
	\end{align*}
...
\begin{lem}
	...
	\begin{align*}
		n_i(x) \geqslant \frac{\sqrt{2} \nu \minfe (\domfeu^2 - \domfel^2)}{\domfeu \left(\frac{\domfeu^2}{2} + \beta\right)^{1/2}} \frac{\domfel}{\sqrt{\beta\left(1+\frac{1}{\mu}\right)}} \quad\quad\forall x\in[0,1].
	\end{align*}
\end{lem}
...
\myproof{
	...
	\begin{align*}
		n_i(x) 
		\geqslant \int_{w=-g_x(-\domfeu)}^{-g_x(0)} \int_{\tau = \underline{\tau}}^{\overline{\tau}} 2 \nu \minfe \, d\tau dw
...
%		= \frac{a \minfe}{\overline{\ee}^{1/2}} \left[\sqrt{\ee + \beta}\right]_{\underline{\ee}}^{\overline{\ee}}.
	...
	\begin{align*}
		 ...
		 \quad\text{so that}\quad
 		 ...
 		 \geqslant \frac{\domfel}{\sqrt{\beta\left(1+\frac{1}{\mu}\right)}}.
	\end{align*}
...
\begin{lem}[Coarse stability]
	Assume all the hypotheses of the above lemmas are satisfied. \todo{Be more precise.}
	For all $(\lambda,\mu) \in (\mathbb{R}_*^+)^2$, there exists parameters $(\alpha,\beta,\nu) \in (\mathbb{R}_*^+)^3$ such that $F(\K) \subset \K$.
\end{lem}

\myproof{
	The function $F$ is solution to a Poisson problem with bounded continuous source term, so it enjoys $\mathcal{C}^2$ regularity. The boundary condition $F_{\varphi}(0)=F_{\varphi}'(0)=0$ are satisfied by construction. We turn to the stability of the variation estimates: owing to the boundedness of $n_i$ and $n_e$ stated above, we have
	\begin{align*}
		\begin{cases}
			- \lambda^2 F''(x) \leqslant 8 \nu \left(\frac{\beta}{\alpha}\right)^{1/4} \left(\frac{1}{\alpha^{1/4}} + \beta^{1/4} + \frac{1}{\mu^{1/4}}\right), \\
			- \lambda^2 F''(x) \geqslant \nu \frac{\kappa_1}{\sqrt{\beta} \left(\kappa_2+ \beta\right)^{1/2} \sqrt{1+\frac{1}{\mu}}}  - 2 \maxfe \sqrt{\frac{2\beta}{\mu}}
		\end{cases}
	\end{align*}
	where
	\begin{align*}
		\kappa_1 \coloneqq \frac{\minfe (\domfeu^2 - \domfel^2) \domfel}{\sqrt{2}\domfeu}, \quad 
		\kappa_2 \coloneqq \frac{\domfeu^2}{2}
	\end{align*}
	A sufficient condition for the stability is to obtain $\alpha \leqslant -F'' \leqslant \beta$. We will restrict our search to $0 < \alpha \leqslant \beta$ such that $\alpha\beta=1$. Then, sufficient conditions write
	\begin{align*}
		\begin{cases}
			1 \leqslant \beta \\
			8 \frac{\nu}{\lambda^2} \beta^{1/2} \left(\beta^{-1/4} + \beta^{1/4} + \frac{1}{\mu^{1/4}}\right) \leqslant \beta \\
			\nu \frac{\kappa_1}{\sqrt{\beta} \left(\kappa_2+ \beta\right)^{1/2} \sqrt{1+\frac{1}{\mu}}}  - 2 \maxfe \sqrt{\frac{2\beta}{\mu}} \geqslant \beta^{-1}
		\end{cases}
		\quad\iff\quad
		\begin{cases}
			1 \leqslant \beta \\
			1 + \beta^{1/2} + \frac{\beta^{1/4}}{\mu^{1/4}} - \frac{\lambda^2}{8 \nu} \beta \leqslant 0 \\
			\nu \frac{\kappa_1 \sqrt{\beta} }{\left(\kappa_2+ \beta\right)^{1/2} \sqrt{1+\frac{1}{\mu}}}  \geqslant 1 + 2 \maxfe \sqrt{\frac{2}{\mu}} \beta^{3/2}
		\end{cases}
	\end{align*}
	Using the coarse estimate $\beta^{1/4} \leqslant \beta^{1/2}$ on $1 \leqslant \beta$ yields
	\begin{align*}
		1 + \beta^{1/2} + \frac{\beta^{1/4}}{\mu^{1/4}} - \frac{\lambda^2}{8 \nu} \beta 
		\leqslant 1 + \beta^{1/2} \left(1 + \frac{1}{\mu^{1/4}}\right) - \frac{\lambda^2}{8 \nu} \beta 
		\coloneqq 1 + X \left(1 + \frac{1}{\mu^{1/4}}\right) - \frac{\lambda^2}{8 \nu} X^2,
	\end{align*}
	and this concave polynomial is negative for $\beta$ large enough. \todo{Do the computation if useful}
	
	Moreover, since $1 \geqslant \beta$ and the function $\beta \to \frac{\sqrt{\beta}}{\sqrt{\kappa_2 + \beta}}$ is increasing, the third condition is implied by
	\begin{align*}
		\nu \frac{\kappa_1}{\left(\kappa_2+ 1\right)^{1/2} \sqrt{1+\frac{1}{\mu}}}  \geqslant 1 + 2 \maxfe \sqrt{\frac{2}{\mu}} \beta^{3/2}.
	\end{align*}
}
...
\item If the estimates with respect to $|\varphi - \psi|_{\infty}$ succeed, define numerical scheme and see what we can say about it.
\end{itemize}
}
