...
\author{
	Valentin Ayot\footnote{Institut de Mathématiques, CNRS, UMR 5251, Université de Bordeaux, F-33405 Talence, France. \texttt{valentin.ayot@u-bordeaux.fr}}, 
	 \ Averil Prost\footnote{INSA de Rouen, LMI (EA 3226 - FR CNRS 3335), 685 Avenue de l'Université, 76801 St Etienne du Rouvray cedex, France. \texttt{averil.prost@insa-rouen.fr}}, 
	 \ Christian Tayou-Fotso\footnote{Labo. J. A. Dieudonné, UMR 6621, Université Nice-Sophia Antipolis, Parc Valrose, F-06108 Nice cedex 02, France. \texttt{christian.tayou-fotso@unice.fr}}.
 } 
\date{}
...
The purpose of this work is to investigate numerically the formation of this sheath. \todo{Work on biblio!} It follows previous work of \todo{ref travail Michel et Yann} on a periodic case, and of \todo{ref Mehdi, Anaïs} on a collisional model. Our aim is to study an ionization model with nonperiodic boundary conditions. The first section introduces the corresponding system, and derives the boundary conditions. The second section presents the numerical methods used to simulate the evolution of the plasma, and the third section gives some results and comparisons.
...
We rely on the work of \cite{malkov2020} to provide an analytical solution in a 1-species case. Consider the simplified stationary model describing the density of particles $f = f(t,x,v)$, and the potential $\varphi=\varphi(t,x)$:
...
\begin{align}\label{eq:Malkov_solution}
	f(t,x,v) \coloneqq 
	\begin{cases}
		\frac{1}{\pi} \left(1 - x^2 - v^2\right)^{-1/2} & \text{if } x^2 + z^2 < 1 \\
		0 & \text{otherwise}
...
\begin{align}\label{eq:Malkov_solution_ext}
	...
	\begin{cases}
		...
		0 & \text{otherwise}
	\end{cases}, \quad \text{and} \quad
	\varphi(t,x) \coloneqq \min\left(\frac{x^2}{2}, \frac{|x|}{2}\right).
\end{align}

\Cref{fig:malkov_solutions} illustrates the stationary solutions.

% Simulations to have : 
% - both codes, same parameters 
% - please show convergence...

\begin{figure}
	\centering
	\includegraphics[height=0.25\linewidth]{images/malkov_solution_Ee}
	\includegraphics[height=0.25\linewidth]{images/malkov_solution_fe}
