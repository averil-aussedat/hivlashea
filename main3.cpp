...
double vmin_e  = -10./sqrt(mu);    //-10./sqrt(mu);
double  vmax_e = 10./sqrt(mu);      //10./sqrt(mu);
double  dx     = (xmax - xmin)/Nx;
double  dv     = (vmax_e  -vmin_e)/Nv;
double  CFL_x  = 0.5* dx/vmax_e;
double  CFL_v  = mu*dv/10;   // mu*dv/10;
...
int main(int argc, char ** argv)
{
    ...
    Matrix update_fi(Matrix &,Matrix &, Vector&); Matrix update_fe(Matrix&,Vector&); //,Matrix&,Matrix&
    void plot_sol(Matrix&,Matrix& , Vector &, Vector&, int &);      //plot_sol(Matrix&, Vector &, int &);          //plot_sol(Matrix&,Matrix& , Vector &, Vector&, int &);
    ...
    while( tn < T )
    {
        ...
        E  = compute_eletric_field(rho,E,J_xmin,J_xmax,Me);tE = -(1/mu)*E;       //E;      //compute_eletric_field(rho,E,J_xmin,J_xmax,Me);tE = -(1/mu)*E;
        plot_sol(fi,fe,rho,E,iter); //plot_sol(fe,E,iter);  // plot_sol(fi,fe,rho,E,iter);
        fi  = update_fi(fi,fe,E);
        fe =  update_fe(fe,E); //,fe,fe
        tn+=dt;
...
void plot_sol(Matrix & fii,Matrix & fee, Vector & rho, Vector & E, int & iter)  // Matrix & fii,Matrix & fee, Vector & rho, Vector & E, int & iter
{
...
{   
    ...
    for(int j = 0; j < Nv  ; j++)
    {
        ...
        if( vmin_e + j*dv > 0)
        {
            while( tn < T )
            {
                new_fi[0][j] = fe_L(vmin_e + j*dv,tn);
                tn+=dt;
    ...
    for(int j = 0; j < Nv ; j++)
    {
        ...
        if( vmin_e + j*dv < 0)
        {
            while( tn < T )
            {
                new_fi[Nx][j] = fe_R(vmin_e + j*dv,tn);
                tn+=dt;
            }
        }
        else
...
Matrix update_fe(Matrix & fe_0, Vector & E)   //,Matrix & fe_L,Matrix & fe_R
{
    double tn(0);
    ...
    for(int j = 0; j < Nv  ; j++)
    {
        ...
        if( vmin_e + j*dv > 0)
        {
            while( tn < T )
            {
                new_fe[0][j] = fe_L(vmin_e + j*dv,tn);
                tn+=dt;
    ...
    for(int j = 0; j < Nv ; j++)
    {
        ...
        if( vmin_e + j*dv < 0)
        {
            while( tn < T )
            {
                new_fe[Nx][j] = fe_R(vmin_e + j*dv,tn);
                tn+=dt;
            }
