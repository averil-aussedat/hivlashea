...
double vmin_e  = -1.0;    //-10./sqrt(mu);
double  vmax_e = 1.0;      //10./sqrt(mu);
double  dx     = (xmax - xmin)/Nx;
double  dv     = (vmax_e  -vmin_e)/Nv;
double  CFL_x  = 0.5* dx/vmax_e;
double  CFL_v  = 0.5*dv/xmax;   // mu*dv/10;
...
int main(int argc, char ** argv)
{
    ...
    void plot_sol(Matrix&, Vector &, int &);          //plot_sol(Matrix&,Matrix& , Vector &, Vector&, int &);
    ...
    while( tn < T )
    {
       ...
       // E   = E;      //compute_eletric_field(rho,E,J_xmin,J_xmax,Me);tE = -(1/mu)*E;
        plot_sol(fe,E,iter);  // plot_sol(fi,fe,rho,E,iter);
...
void plot_sol(Matrix & fee, Vector & E, int & iter)  // Matrix & fii,Matrix & fee, Vector & rho, Vector & E, int & iter
{
    ...
    for(int i = 0 ; i < Nx +1; i++)
    {
        for(int j = 0 ; j < Nv +1 ; j++)
        {                                                    // " << fii[i][j] << "
            file  << xmin + i * dx << "  " << vmin_e + j*dv <<"  "<< fee[i][j] <<endl;
        }
        file << endl;
    }
    file.close();
